//gcc mjr_audio_video_webm.c -o pmpp -ljansson -lavfilter -lavutil -lswresample -lswscale -lavdevice $(pkg-config --cflags --libs libavformat libavcodec) -lpostproc -lpthread `pkg-config --cflags --libs glib-2.0` `pkg-config --cflags --libs opus` `pkg-config --cflags --libs ogg` -levent -lrabbitmq -Wall -g
#include <arpa/inet.h>
#ifdef __MACH__
