...
static int open_output_file(const char *filename)
{
	...
	enc_ctx = out_stream->codec;
        ...
        encoder = avcodec_find_encoder(AV_CODEC_ID_H264); //h264_nvenc
...
void janus_echotest_create_session(janus_plugin_session *handle, int *error) {
	...
	//const char *filter_descr = "drawtext=fontfile=arial.ttf:fontcolor=green:fontsize=30:text='Powered by Dialoga'";
        char args[512];
        AVFilterGraph *filter_graph;
                                            ...
                                            if(session->keyframe_number == 1){
                                                ...
                                                if(fctx) {
                                                    ...
                                                    while ((img = vpx_codec_get_frame(&de_codec, &iter)) != NULL) {
                                                        ...
                                                        ret = enc_func(ofmt_ctx->streams[0]->codec, &enc_pkt,
                                                                            frame_out, &enc_got_frame);
                                                        if (ret < 0)
                                                        {
                                                        ...
                                                        enc_pkt.pts = (session->previous_ts-session->keyframe_ts)/90;
