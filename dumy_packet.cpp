...
typedef struct packet_audio {
    janus_pp_frame_packet *audio_pkt;
...
static gint rtp_sort(gconstpointer a, gconstpointer b, gpointer user_data) {
	janus_pp_frame_packet *pkt1 = (janus_pp_frame_packet *)a;
	janus_pp_frame_packet *pkt2 = (janus_pp_frame_packet *)b;
...
int audio_mix_process_1(file_combine_list *file_combine_list_1) {
    ...
    for (i = 0; i < length; i++) {
        janus_pp_frame_packet *pkt = (janus_pp_frame_packet *)first->data;
        //printf("ts % "SCNu64"\n", pkt->ntp_ms);
        first = first->next;
    }
    return 0;
...
int audio_mix_process(file_combine_list *file_combine_list_1) {
    if(!file_combine_list_1)
    	return -1;
    file_combine *file_combine_1 = file_combine_list_1->head;
    // after decoding check the timestamp
    janus_pp_frame_packet *lowest_tmp;
    
    janus_pp_frame_packet packet_sort[file_combine_list_1->size];
    int i;
    int dif;
    packet_audio_list *packet_audio_list_1 = (packet_audio_list *)g_malloc0(sizeof(packet_audio_list));
    packet_audio_list_1->size = 0;
    packet_audio_list_1->head = NULL;
    packet_audio_list_1->tail = NULL;   
    for( i = 0; i< file_combine_list_1->size; i++) {
        packet_audio *packet_audio_1 = (packet_audio *)g_malloc0(sizeof(packet_audio));
        file_av *file_av_1 = file_combine_1->file_av_list_1->head;
        printf("Counter % "SCNu64"\n", file_av_1->count);
        printf("last spain % "SCNu64"\n", file_av_1->last->ntp_ms);
        if(i == 0) {
            lowest_tmp = file_av_1->list;
            lowest_tmp->dif = 0;
            //lowest_tmp->last_tmp = (file_av_1->count*20) + file_av_1->list->
            printf("lowest first ts % "SCNu64"\n", lowest_tmp->ntp_ms);
        } else {
            if(lowest_tmp->ntp_ms > file_av_1->list->ntp_ms) { 
                lowest_tmp = file_av_1->list;
                printf("lowest second ts % "SCNu64"\n", lowest_tmp->ntp_ms);
            } 
        }
        packet_audio_1->audio_pkt = file_av_1->list;
        packet_sort[i] = *file_av_1->list;
        file_combine_1 = file_combine_1->next;
        if (packet_audio_list_1->head) {
            // Binding the node to the list elements.
            packet_audio_1->next = packet_audio_list_1->head;
            packet_audio_1->prev = packet_audio_list_1->head->prev;
            // Binding the list elements to the node.
            packet_audio_list_1->head->prev->next = packet_audio_1;
            packet_audio_list_1->head->prev = packet_audio_1;
        } else {
            packet_audio_1->next = packet_audio_1;
            packet_audio_1->prev = packet_audio_1;
            packet_audio_list_1->tail = packet_audio_1;
        }
        packet_audio_list_1->head = packet_audio_1;
        packet_audio_list_1->size++;
    }
    GQueue *inbuf = g_queue_new ();
    g_queue_init (inbuf);
    for (i = 0; i < file_combine_list_1->size; i++) {
    ...
    */
    for (i = 0; i < length; i++) {
        janus_pp_frame_packet *pkt = &packet_sort[i];
        printf("ts % "SCNu64"\n", pkt->dif);
        printf("mod % "SCNu64"\n", pkt->mod);
        if(pkt->dif != 0 || pkt->mod != 0) {
            int mod =  pkt->mod;
    ...
    for( i = 0; i< file_combine_list_1->size; i++) {
        ...
        for (xm = 0; xm < sk; xm++) {
            janus_pp_frame_packet *pkt_dummy = (janus_pp_frame_packet *)g_malloc0(sizeof(janus_pp_frame_packet));
            counter = counter + 1;
        ...
        /*if(pkt->dif == 0)
            pkt->counter = 0;
        */
        //janus_pp_frame_packet *pkt1 = (janus_pp_frame_packet *)g_malloc0(sizeof(janus_pp_frame_packet));
        //pkt1 = pkt;
        //g_queue_push_tail(inbuf_added,pkt1);
        
        first->data = pkt;
        first = first->next;
    }
    //first = inbuf_added->head;
    first = inbuf->head;
 /*   janus_pp_frame_packet *pkt = (janus_pp_frame_packet *)first->data;
    printf("counter % "SCNu64"\n", inbuf_added->length);
    printf("counter % "SCNu64"\n", );
   ...
   */ 
    ...
    while(1) {
        for( i = 0; i< file_combine_list_1->size; i++) {
            packet_audio *packet_audio_1 = packet_audio_list_1->head;
            if(count == 0) {
                if(lowest_tmp->ntp_ms < packet_audio_1->audio_pkt->ntp_ms) {
                    //write the packet
                }
                lowest_tmp->ntp_ms = packet_audio_1->audio_pkt->ntp_ms;
            } else {
                if(i = (file_combine_list_1->size-1) {
                    
                }
            }
            count++;
            packet_audio_1 = packet_audio_1->next;
        }
    }
    *//*
    while(1) {
        for( i = 0; i< file_combine_list_1->size; i++) {
            if(lowest_tmp->ntp_ms )
            packet_sort[i] = packet_sort[i].next; 
        }
    }*/
    return 0;
}
...
int main(int argc, char *argv[])
{
	...
	if(strcasecmp(extension, ".webm")) {
        ...
        file_combine_list *file_combine_list_1 = (file_combine_list *)g_malloc0(sizeof(file_combine_list_1));
	...
	if(janus_pp_webm_create(destination) < 0) {
       ...
       */ 
        if(audio_mix_process(file_combine_list_1) < 0) {
