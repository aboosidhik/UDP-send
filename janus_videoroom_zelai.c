...
record_rtcp = true|false (whether the rtcp packets in the room to be recorded, default=false) //Abu
...
static struct janus_json_parameter create_parameters[] = {
	...
	{"record", JANUS_JSON_BOOL, 0},
        {"record_rtcp", JANUS_JSON_BOOL, 0}, //Abu
...
typedef struct janus_videoroom {
	...
	gboolean record;			/* Whether the feeds from publishers in this room should be recorded */
        gboolean record_rtcp;			/* Whether the rtcp packets from publishers in this room should be recorded */ //Abu
...
typedef struct janus_videoroom_participant {
	...
	gboolean recording_active;	/* Whether this publisher has to be recorded or not */
        gboolean recording_rtcp_active;	/* Whether this publisher has to be rtcp packets to be recorded or not */ //Abu
	...
	janus_recorder *vrc;	/* The Janus recorder instance for this publisher's video, if enabled */
        janus_recorder *rtcp_audio;	/* The Janus recorder instance for this publisher's data, if enabled */ //Abu
        janus_recorder *rtcp_video;	/* The Janus recorder instance for this publisher's data, if enabled */ //Abu
	...
	if(config != NULL) {
		...
		while(cl != NULL) {
			...
			janus_config_item *record = janus_config_get_item(cat, "record");
                        janus_config_item *record_rtcp = janus_config_get_item(cat, "record_rtcp"); //Abu
			...
			if(record && record->value) {
				videoroom->record = janus_is_true(record->value);
                        ...
                        if(record_rtcp && record_rtcp->value) {       //Abu
				videoroom->record_rtcp = janus_is_true(record_rtcp->value);  //Abu
			} //Abu
			...
			if(videoroom->record) {
				JANUS_LOG(LOG_VERB, "  -- Room is going to be recorded in %s\n", videoroom->rec_dir ? videoroom->rec_dir : "the current folder");
                        ...
                        if(videoroom->record_rtcp) {  //abu
				JANUS_LOG(LOG_VERB, "  -- RTCP packets in Room is going to be recorded in %s\n", videoroom->rec_dir ? videoroom->rec_dir : "the current folder"); //abu
			} //abu
			cl = cl->next;
		}
	...
	if(!session->destroyed) {
		...
		if(session->participant_type == janus_videoroom_p_type_publisher) {
			...
			participant->recording_active = FALSE;
                        participant->recording_rtcp_active = FALSE; //Abu
				...
				if(participant->arc || participant->vrc) {
					...
					if(participant->vrc && participant->vrc->filename)
						json_object_set_new(recording, "video", json_string(participant->vrc->filename));
                                        if(participant->rtcp_audio && participant->rtcp_audio->filename) //Abu
						json_object_set_new(recording, "rtcp_audio", json_string(participant->rtcp_audio->filename)); //Abu
                                        if(participant->rtcp_video && participant->rtcp_video->filename) //Abu
						json_object_set_new(recording, "rtcp_video", json_string(participant->rtcp_video->filename)); //Abu        
...
struct janus_plugin_result *janus_videoroom_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep) {
	...
	if(!strcasecmp(request_text, "create")) {
		...
		json_t *record = json_object_get(root, "record");
                json_t *record_rtcp = json_object_get(root, "record_rtcp"); //Abu
		...
		if(record) {
			videoroom->record = json_is_true(record);
                ...
                if(record_rtcp) {   //Abu
			videoroom->record_rtcp = json_is_true(record_rtcp); //Abu
		} //Abu
		if(rec_dir) {
		...
		if(save) {
			...
			if(videoroom->record)
				janus_config_add_item(config, cat, "record", "yes");
                        if(videoroom->record_rtcp) //abu
				janus_config_add_item(config, cat, "record_rtcp", "yes"); //abu
		...
		while(g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			if(!room->destroyed) {
				...
				json_object_set_new(rl, "record", room->record ? json_true() : json_false());
                                json_object_set_new(rl, "record_rtcp", room->record_rtcp ? json_true() : json_false()); //abu
...
void janus_videoroom_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	if(session->destroyed)
		return;
        janus_videoroom_participant *participant = (janus_videoroom_participant *)session->participant; //abu
	janus_videoroom *videoroom = participant->room; //abu
        /* Save the frame if we're recording */
	...
	} else if (session->participant_type == janus_videoroom_p_type_publisher) { //abu
            janus_recorder_save_frame(video ? participant->rtcp_video : participant->rtcp_audio, buf, len); //abu
        } //abu
}
		...
		if(participant->recording_base) {
			...
			if(participant->arc == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an audio recording file for this publisher!\n");
                        ...
                        g_snprintf(filename, 255, "%s-rtcp_audio", participant->recording_base); //abu
			participant->rtcp_audio = janus_recorder_create(participant->room->rec_dir, //abu
				janus_videoroom_audiocodec_name(participant->room->acodec), filename); //abu
			if(participant->rtcp_audio == NULL) { //abu
				JANUS_LOG(LOG_ERR, "Couldn't open an audio recording file for this publisher!\n"); //abu
			} //abu
		} else {
			/* Build a filename */
			...
			if(participant->arc == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an audio recording file for this publisher!\n");
                        ...
                        /* Build a filename */
			g_snprintf(filename, 255, "videoroom-%"SCNu64"-user-%"SCNu64"-%"SCNi64"-rtcp_audio", //abu
				participant->room->room_id, participant->user_id, now); //abu
			participant->rtcp_audio = janus_recorder_create(participant->room->rec_dir, //abu
				janus_videoroom_audiocodec_name(participant->room->acodec), filename); //abu
			if(participant->rtcp_audio == NULL) { //abu
				JANUS_LOG(LOG_ERR, "Couldn't open an audio recording file for this publisher!\n"); //abu
		...
		if(participant->recording_base) {
			...
			if(participant->vrc == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an video recording file for this publisher!\n");
                        ...
                        /* Use the filename and path we have been provided */
			g_snprintf(filename, 255, "%s-rtcp_video", participant->recording_base); //abu
			participant->rtcp_video = janus_recorder_create(participant->room->rec_dir, //abu
				janus_videoroom_videocodec_name(participant->room->vcodec), filename); //abu
			if(participant->rtcp_video == NULL) { //abu
				JANUS_LOG(LOG_ERR, "Couldn't open an video recording file for this publisher!\n"); //abu
			} //abu
		} else {
			/* Build a filename */
			...
			if(participant->vrc == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an video recording file for this publisher!\n");
                        ...
                        /* Build a filename */
			g_snprintf(filename, 255, "videoroom-%"SCNu64"-user-%"SCNu64"-%"SCNi64"-rtcp_video", //abu
				participant->room->room_id, participant->user_id, now); //abu
			participant->rtcp_video = janus_recorder_create(participant->room->rec_dir, //abu
				janus_videoroom_videocodec_name(participant->room->vcodec), filename); //abu
			if(participant->rtcp_video == NULL) { //abu
...
static void janus_videoroom_recorder_close(janus_videoroom_participant *participant) {
	...
	participant->arc = NULL;
        if(participant->rtcp_audio) { //abu
		janus_recorder_close(participant->rtcp_audio); //abu
		JANUS_LOG(LOG_INFO, "Closed audio recording %s\n", participant->rtcp_audio->filename ? participant->rtcp_audio->filename : "??"); //abu
		janus_recorder_free(participant->rtcp_audio); //abu
	} //abu
	participant->rtcp_audio = NULL;
	if(participant->vrc) {
	...
	participant->vrc = NULL;
        if(participant->rtcp_video) {  //abu
		janus_recorder_close(participant->rtcp_video); //abu
		JANUS_LOG(LOG_INFO, "Closed video recording %s\n", participant->rtcp_video->filename ? participant->rtcp_video->filename : "??"); //abu
		janus_recorder_free(participant->rtcp_video); //abu
	} //abu
	participant->rtcp_video = NULL; //abu
}
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(session->participant_type == janus_videoroom_p_type_none) {
			...
			if(!strcasecmp(ptype_text, "publisher")) {
				...
				json_t *audio = NULL, *video = NULL, *bitrate = NULL, *record = NULL, *recfile = NULL, *record_rtcp = NULL;
				if(!strcasecmp(request_text, "joinandconfigure")) {
					...
					record = json_object_get(root, "record");
                                        record_rtcp = json_object_get(root, "record_rtcp"); //abu
				...
				publisher->recording_active = FALSE;
                                publisher->recording_rtcp_active = FALSE; //abu
				...
				if(record) { 
					...
					JANUS_LOG(LOG_VERB, "Setting record property: %s (room %"SCNu64", user %"SCNu64")\n", publisher->recording_active ? "true" : "false", publisher->room->room_id, publisher->user_id);
                                ...
                                if(record_rtcp) { //abu
					publisher->recording_rtcp_active = json_is_true(record_rtcp); //abu
					JANUS_LOG(LOG_VERB, "Setting record property: %s (room %"SCNu64", user %"SCNu64")\n", publisher->recording_rtcp_active ? "true" : "false", publisher->room->room_id, publisher->user_id); //abu
				} //abu
				if(recfile) {
			...
			if(!strcasecmp(request_text, "join") || !strcasecmp(request_text, "joinandconfigure")) {
				...
				json_t *bitrate = json_object_get(root, "bitrate");
				json_t *record = json_object_get(root, "record");
                                json_t *record_rtcp = json_object_get(root, "record_rtcp");
				...
				gboolean prev_recording_active = participant->recording_active;
                                gboolean prev_recording_rtcp_active = participant->recording_rtcp_active; //abu
				if(record) {
					...
					JANUS_LOG(LOG_VERB, "Setting record property: %s (room %"SCNu64", user %"SCNu64")\n", participant->recording_active ? "true" : "false", participant->room->room_id, participant->user_id);
                                ...
                                if(record_rtcp) { //abu
					participant->recording_rtcp_active = json_is_true(record_rtcp); //abu
					JANUS_LOG(LOG_VERB, "Setting record property: %s (room %"SCNu64", user %"SCNu64")\n", participant->recording_rtcp_active ? "true" : "false", participant->room->room_id, participant->user_id); //abu
				} //abu
				if(recfile) {
				...
				if(participant->recording_active != prev_recording_active || participant->recording_rtcp_active != prev_recording_rtcp_active) { //abu
					/* Something changed */
					if(!participant->recording_active || participant->recording_rtcp_active) { //abu
						/* Not recording (anymore?) */
						janus_videoroom_recorder_close(participant);
					} else if((participant->recording_active || participant->recording_rtcp_active) && participant->sdp) { //abu
		...
		if(!msg_sdp) {
			...
			if(session->participant_type != janus_videoroom_p_type_publisher) {
				...
				for(pass = 1; pass <= 2; pass++) {
					...
					janus_mutex_lock(&participant->rec_mutex);
					if(videoroom->record || participant->recording_active || participant->recording_rtcp_active) {
...
static void janus_videoroom_participant_free(janus_videoroom_participant *p) {
	...
	if(p->arc) {
        ...
        if(p->rtcp_audio) { //abu
		janus_recorder_free(p->rtcp_audio); //abu
		p->rtcp_audio = NULL; //abu
	} //abu
        if(p->vrc) {
        ...
        if(p->rtcp_video) { //abu
		janus_recorder_free(p->rtcp_video); //abu
		p->rtcp_video = NULL; //abu
	} //abu   
