...
record_rtcp = true|false (whether the rtcp packets in the room to be recorded, default=false)
...
typedef struct janus_videoroom {
	...
	gboolean record;			/* Whether the feeds from publishers in this room should be recorded */
        gboolean record_rtcp;			/* Whether the rtcp packets from publishers in this room should be recorded */
...
typedef struct janus_videoroom_participant {
	...
	gboolean recording_active;	/* Whether this publisher has to be recorded or not */
        gboolean recording_rtcp_active;	/* Whether this publisher has to be rtcp packets to be recorded or not */
	...
	janus_recorder *drc;	/* The Janus recorder instance for this publisher's data, if enabled */
        janus_recorder *rtcp_audio;	/* The Janus recorder instance for this publisher's data, if enabled */
        janus_recorder *rtcp_video;	/* The Janus recorder instance for this publisher's data, if enabled */
	...
	if(config != NULL) {
		...
		while(cl != NULL) {
			...
			janus_config_item *record = janus_config_get_item(cat, "record");
                        janus_config_item *record_rtcp = janus_config_get_item(cat, "record_rtcp");
			...
			if(record && record->value) {
				videoroom->record = janus_is_true(record->value);
                        ...
                        if(record_rtcp && record_rtcp->value) {
				videoroom->record_rtcp = janus_is_true(record_rtcp->value);
			}
	...
	if(!session->destroyed) {
		...
		if(session->participant_type == janus_videoroom_p_type_publisher) {
			...
			participant->recording_active = FALSE;
                        participant->recording_rtcp_active = FALSE;
	...
	if(session->participant) {
		if(session->participant_type == janus_videoroom_p_type_none) {
			...
			if(participant) {
				...
				if(participant->arc || participant->vrc || participant->drc || participant->rtcp_audio || participant->rtcp_video) {
					...
					if(participant->drc && participant->drc->filename)
						json_object_set_new(recording, "data", json_string(participant->drc->filename));
                                        if(participant->rtcp_audio && participant->rtcp_audio->filename)
						json_object_set_new(recording, "rtcp_audio", json_string(participant->rtcp_audio->filename));
                                        if(participant->rtcp_video && participant->rtcp_video->filename)
						json_object_set_new(recording, "rtcp_video", json_string(participant->rtcp_video->filename));                                        
	...
	if(!strcasecmp(request_text, "create")) {
		...
		if(record) {
			videoroom->record = json_is_true(record);
                ...
                if(record_rtcp) {
			videoroom->record_rtcp = json_is_true(record_rtcp);
		}
		if(rec_dir) {
		...
		if(save) {
			...
			if(videoroom->record)
				janus_config_add_item(config, cat, "record", "yes");
                        if(videoroom->record_rtcp)
				janus_config_add_item(config, cat, "record_rtcp", "yes");
		...
		while(g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			if(!room->destroyed) {
				...
				json_object_set_new(rl, "record", room->record ? json_true() : json_false());
                                json_object_set_new(rl, "record_rtcp", room->record_rtcp ? json_true() : json_false());
...
void janus_videoroom_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len) {
	...
	if(session->participant_type == janus_videoroom_p_type_subscriber) {
		...
		if(bitrate > 0) {
            ...
            janus_recorder_save_frame(video ? participant->rtcp_video : participant->rtcp_audio, buf, len);
        }
}
		...
		if(participant->recording_base) {
			...
			if(participant->arc == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an audio recording file for this publisher!\n");
                        ...
                        g_snprintf(filename, 255, "%s-rtcp_audio", participant->recording_base);
			participant->rtcp_audio = janus_recorder_create(participant->room->rec_dir,
				janus_videoroom_audiocodec_name(participant->room->acodec), filename);
			if(participant->rtcp_audio == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an audio recording file for this publisher!\n");
			}
		} else {
			/* Build a filename */
			...
			if(participant->arc == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an audio recording file for this publisher!\n");
                        ...
                        /* Build a filename */
			g_snprintf(filename, 255, "videoroom-%"SCNu64"-user-%"SCNu64"-%"SCNi64"-rtcp_audio",
				participant->room->room_id, participant->user_id, now);
			participant->rtcp_audio = janus_recorder_create(participant->room->rec_dir,
				janus_videoroom_audiocodec_name(participant->room->acodec), filename);
			if(participant->rtcp_audio == NULL) {
		...
		if(participant->recording_base) {
			...
			if(participant->vrc == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an video recording file for this publisher!\n");
                        ...
                        /* Use the filename and path we have been provided */
			g_snprintf(filename, 255, "%s-rtcp_video", participant->recording_base);
			participant->rtcp_video = janus_recorder_create(participant->room->rec_dir,
				janus_videoroom_videocodec_name(participant->room->vcodec), filename);
			if(participant->rtcp_video == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an video recording file for this publisher!\n");
			}
		} else {
			/* Build a filename */
			...
			if(participant->vrc == NULL) {
				JANUS_LOG(LOG_ERR, "Couldn't open an video recording file for this publisher!\n");
                        ...
                        /* Build a filename */
			g_snprintf(filename, 255, "videoroom-%"SCNu64"-user-%"SCNu64"-%"SCNi64"-rtcp_video",
				participant->room->room_id, participant->user_id, now);
			participant->rtcp_video = janus_recorder_create(participant->room->rec_dir,
				janus_videoroom_videocodec_name(participant->room->vcodec), filename);
			if(participant->rtcp_video == NULL) {
...
static void janus_videoroom_recorder_close(janus_videoroom_participant *participant) {
	...
	participant->arc = NULL;
       if(participant->rtcp_audio) {
		janus_recorder_close(participant->rtcp_audio);
		JANUS_LOG(LOG_INFO, "Closed audio recording %s\n", participant->rtcp_audio->filename ? participant->rtcp_audio->filename : "??");
		janus_recorder_free(participant->rtcp_audio);
	}
	...
	participant->vrc = NULL;
        if(participant->rtcp_video) {
		janus_recorder_close(participant->rtcp_video);
		JANUS_LOG(LOG_INFO, "Closed video recording %s\n", participant->rtcp_video->filename ? participant->rtcp_video->filename : "??");
		janus_recorder_free(participant->rtcp_video);
	}
	participant->rtcp_video = NULL;
	if(participant->drc) {
		...
		if(session->participant_type == janus_videoroom_p_type_none) {
			...
			if(!strcasecmp(ptype_text, "publisher")) {
				...
				json_t *audio = NULL, *video = NULL, *data = NULL,
					*bitrate = NULL, *record = NULL, *recfile = NULL, *record_rtcp = NULL;
				if(!strcasecmp(request_text, "joinandconfigure")) {
					...
					record = json_object_get(root, "record");
                                        record_rtcp = json_object_get(root, "record_rtcp");
				...
				publisher->recording_active = FALSE;
                                publisher->recording_rtcp_active = FALSE;
				...
				if(record) {
					...
					JANUS_LOG(LOG_VERB, "Setting record property: %s (room %"SCNu64", user %"SCNu64")\n", publisher->recording_active ? "true" : "false", publisher->room->room_id, publisher->user_id);
                               ...
                               if(record_rtcp) {
					publisher->recording_rtcp_active = json_is_true(record_rtcp);
					JANUS_LOG(LOG_VERB, "Setting record property: %s (room %"SCNu64", user %"SCNu64")\n", publisher->recording_rtcp_active ? "true" : "false", publisher->room->room_id, publisher->user_id);
				}
				if(recfile) {
			...
			if(!strcasecmp(request_text, "join") || !strcasecmp(request_text, "joinandconfigure")) {
				...
				gboolean prev_recording_active = participant->recording_active;
                                gboolean prev_recording_rtcp_active = participant->recording_rtcp_active;
				if(record) {
					...
					JANUS_LOG(LOG_VERB, "Setting record property: %s (room %"SCNu64", user %"SCNu64")\n", participant->recording_active ? "true" : "false", participant->room->room_id, participant->user_id);
                                ...
                                if(record_rtcp) {
					participant->recording_rtcp_active = json_is_true(record_rtcp);
					JANUS_LOG(LOG_VERB, "Setting record property: %s (room %"SCNu64", user %"SCNu64")\n", participant->recording_rtcp_active ? "true" : "false", participant->room->room_id, participant->user_id);
				}
				if(recfile) {
				...
				if(participant->recording_active != prev_recording_active || participant->recording_rtcp_active != prev_recording_rtcp_active) {
					/* Something changed */
					if(!participant->recording_active || participant->recording_rtcp_active) {
						/* Not recording (anymore?) */
						janus_videoroom_recorder_close(participant);
					} else if((participant->recording_active || participant->recording_rtcp_active) && participant->sdp) {
...
static void janus_videoroom_participant_free(janus_videoroom_participant *p) {
	...
	if(p->arc) {
        ...
        if(p->rtcp_audio) {
		janus_recorder_free(p->rtcp_audio);
		p->rtcp_audio = NULL;
	}
	if(p->vrc) {
        ...
        if(p->rtcp_video) {
		janus_recorder_free(p->rtcp_video);
		p->rtcp_video = NULL;
	}        
	if(p->drc) {
