...
AVFrame* useOpenCVProcessFrame(AVFrame *frame, int stream_index)
{
   ...
   /* cuda::GpuMat d_src_image(image_re);
    ...
    cuda::resize(d_src, d_dst1, Size(imageFrame.cols/2,imageFrame.rows/2), 0, 0, INTER_CUBIC);  
    cuda::resize(d_src, d_dst2, Size(imageFrame.cols/2,imageFrame.rows/2), 0, 0, INTER_CUBIC);  
    cuda::resize(d_src, d_dst_big, Size(imageFrame.cols/2,imageFrame.rows), 0, 0, INTER_CUBIC);  
   ...
   */// d_src_image.download(imageFrame);
    ...
    while (1) {
        ...
        if (type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (got_frame) {
                ...
                srcFrame->pts = av_frame_get_best_effort_timestamp(frame);
                srcFrame->pict_type =  AV_PICTURE_TYPE_NONE;
                ret = encode_write_frame(srcFrame, stream_index, NULL);
#else
